...
package object example {
  ...
  abstract class EventStoreInMemory[ID, PAYLOAD]() {
    ...
    private def currentSequence(idProfessionelSante: ID) = get(idProfessionelSante).size
    def get(idProfessionelSante: ID) = repo
      .filter(_._1 == idProfessionelSante)
      .map(_._2)

    def store(idProfessionelSante: ID, eventList: List[PAYLOAD], knownSequence: Int) = {
      if (currentSequence(idProfessionelSante) == knownSequence)
